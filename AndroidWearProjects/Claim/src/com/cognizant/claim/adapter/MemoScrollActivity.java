...
import com.cognizant.claim.MemoScrollMenuActivity;
import com.cognizant.claim.NotesService;
...
public class MemoScrollActivity extends Activity {
	...
	public void onResume() {
		...
		if (adapter.getCount() > memoList.size()) {
			...
			// refresh the service. This allows the list in the live card to
			// update
	...
	private void createCards() {
		...
		for (int i = 0; i < memoList.size(); i++) {
			...
			tempMemoCard.setFootnote("Note " + (i + 1) + " of "
					+ memoList.size());
			mCards.add(tempMemoCard);
		}
	}

	// Gesture recognizer. this will handle opening the menu when a card is
	// clicked
		...
		gestureDetector.setBaseListener(new GestureDetector.BaseListener() {
			...
			public boolean onGesture(Gesture gesture) {
				if (gesture == Gesture.LONG_PRESS) {
					...
					Intent memoScrollMenuIntent = new Intent(context,
							MemoScrollMenuActivity.class);
					// pass card index and card text to the intent
					memoScrollMenuIntent.putExtra("scrollposition",
							mCardScrollView.getSelectedItemPosition())
							.putExtra(
									"memotext",
									memoList.get(mCardScrollView
